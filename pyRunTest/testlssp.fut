...
let redOp (pind : int)
  ...
  let connect = if (tlx == 0) || (tly == 0) then true else pred2(pind, lastx, firsty)
  let newlss = if connect then max ((max (lssx, (lcsx+lisy))), lssy) else max (lssx, lssy)
  let newlis = if tlx == 0 then lisy else if (tlx == lisx) && connect then lisx+lisy else lisx
  let newlcs = if tly == 0 then lcsx else if (tly == lcsy) && connect then lcsx+lcsy else lcsy
  let newtl  = tlx + tly
