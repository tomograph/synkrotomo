...
let postprocess_fp [a][f][s](angles: [a]f32) (val_flat: [f]f32) (val_steep: [s]f32) (numrhos: i32): []f32 =
  ...
  let flat_indexes = flatten (map(\angle_idx -> map(\rho -> angle_idx*numrhos + rho)(iota numrhos)) flat_angle_indexes)
  let steep_indexes = flatten (map(\angle_idx -> map(\rho -> angle_idx*numrhos + rho)(iota numrhos)) steep_angle_indexes)
