...
let SIRTcb [n][p](angles : []f32)
          ...
          let inversecolumnsums = inverse (cur_best angles rhozero deltarho size (replicate p 1))
          let res = loop (image) = (image) for iter < iterations do
               ...
               (image with [0:n] = (map2 (+) image (map2 (*) inversecolumnsums (cur_best angles rhozero deltarho size (map2 (*) inverserowsums (map2 (-) projections (forward_projection angles rhos halfsize image)))))))
          in res
