...
[ (0.7,1),(0.04,5),(-1.0,-1),(-1.0,-1)],
[...                                  ],
[...                                  ],
[...                                  ]]
\end{lstlisting}
  \caption{An example of the system matrix format - semisparse with a width of $2\cdot n -1$.}
\end{figure}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Flattening}
  Forward projection is handled as a matrix vector multiplication. The translation from nested to flattened is done by considering each nested parallel operator%TODO check code after compile, is it flat
  \begin{lstlisting}[frame=single, language=Futhark]
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Preferably we would write efficient nested, easily flattened code and let the compiler handle flattening %line 2276
  \end{itemize}
\end{frame}
%
% \begin{frame}[fragile]
...
\begin{itemize}
\item{By dumping the compiled code with the --dump command and getting the time spend in different kernels with the -D option we found that most of the time was spent on calculating the system matrix}
...
\end{figure}
\end{frame}
%%% NOTE calculate weight has an if then else based on the angle of the ray, this would be a good oint to pull it out from the inner function and partition to avoid if then else branching waits
 \begin{frame}
 \nocite{*}
