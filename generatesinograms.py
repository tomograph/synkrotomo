...
def main(argv):
    ...
    dp_result = fp_doubleparallel.main( theta_rad.astype(np.float32), rays.astype(np.float32), phantom.flatten().astype(np.float32), 32).get()
    ...
    jh_result = fp_jh.main( theta_rad.astype(np.float32), rays.astype(np.float32), phantom.flatten().astype(np.float32), 32).get()
